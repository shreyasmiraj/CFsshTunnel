    ...
    for _ in range(10):
        try:
            with urllib.request.urlopen(metrics_url) as response:
                ...
                hostname_end_str = "cloudflare.com"
...
def create_cloudflare_tunnel(ssh_port: int, configured_cloudflare: bool, cloudflare_call: str=None, test: bool=False) -> Union[List[str],str]:
    """
    ...
    cf_process = subprocess.Popen(cloudflare_call.split(" "),
    ...
    if test:
        ssh_config_params.insert(len(ssh_config_params),"\tStrictHostKeyChecking no")
